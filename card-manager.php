...
// ===================================================================
// CARD LIBRARY & HAND MANAGEMENT (FIXED LOGIC)
// ===================================================================
...
if (!isset($_SESSION['player_hand'])) {
    // First time - start with empty hand (cards must be drawn from deck)
...
// ===================================================================
// EQUIPMENT STATE TRACKING (NEW)
// ===================================================================
// Initialize equipment state - null means no equipment equipped
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['reset_mechs'])) {
        ...
        // Reset player equipment (enemy keeps theirs)
        $playerEquipment = ['weapon' => null, 'armor' => null];
        // Reset hand to empty (cards must be drawn from deck)
        $_SESSION['player_hand'] = [];
        $playerHand = $_SESSION['player_hand'];
        $gameLog[] = "[" . date('H:i:s') . "] Mechs reset! Player equipment unequipped, hand cleared.";
    }
...
<div class="battlefield-container">
    ...
    <header class="top-bar">
        ...
        <div class="nav-right">
            <a href="build-info.php" class="version-badge">v0.9.1</a>
    ...
    <main class="battlefield">
        ...
        <section class="combat-zone enemy-zone">
            ...
            <div class="battlefield-layout">
                ...
                <div class="draw-deck-area enemy-deck">
                    <div class="draw-pile">
                        <?php for ($i = 0; $i < min(5, count($cardLibrary)); $i++): ?>
                            <div class="draw-card face-down" style="z-index: <?= 5-$i ?>"></div>
                        <?php endfor; ?>
                    </div>
                    <div class="deck-label">Draw (<?= count($cardLibrary) ?>)</div>
                </div>
        ...
        <section class="combat-zone player-zone">
            ...
            <div class="hand-section player-hand-section">
                <div class="hand-label">Your Hand (<?= count($playerHand) ?>/<?= $gameRules['starting_hand_size'] ?>)</div>
                <div class="hand-cards-fan">
                    <?php 
                    // Display real cards up to hand size
                    for ($i = 0; $i < $gameRules['starting_hand_size']; $i++): 
                        if ($i < count($playerHand)):
                            $card = $playerHand[$i];
                    ?>
                        <button type="button" onclick="handleCardClick(<?= $i ?>, '<?= $card['type'] ?>')" class="hand-card face-up fan-card <?= $card['type'] ?>-card" style="--card-index: <?= $i ?>" data-card='<?= htmlspecialchars(json_encode($card), ENT_QUOTES, 'UTF-8') ?>'>
    ...
    <footer class="game-footer">
        <div class="build-info">
            NRD Tactical Sandbox | Game Interface | Build v0.9.1 - Phase 1: Empty Hand + Deck Mechanics (Step 1)
        </div>
    </footer>

</div>

<!-- Hidden forms for equipment actions -->
...
<script>
// ===================================================================
// EQUIPMENT SYSTEM JAVASCRIPT (NEW)
// ===================================================================
